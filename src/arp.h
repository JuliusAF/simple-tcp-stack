...
#define debug_arp_cache(str, entry)                                                            \
    do {                                                                                       \
        printf("arp cache "str" (hwtype: %hu, src_ip: %hhu.%hhu.%hhu.%hhu, "                   \
            ...
            entry->src_mac[0], entry->src_mac[1], entry->src_mac[2], entry->src_mac[3],        \
            entry->src_mac[4], entry->src_mac[5], entry->state);                               \
    } while (0)
#else
#define debug_arp(str, hdr)
#define debug_arp_payload(str, data)
...
static pthread_mutex_t arp_entry_mutex = PTHREAD_MUTEX_INITIALIZER;
static pthread_cond_t arp_entry_cond = PTHREAD_COND_INITIALIZER;
